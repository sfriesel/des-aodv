...
int aodv_periodic_cleanup_database(void *data, struct timeval *scheduled, struct timeval *interval) {
	...
	if (aodv_db_cleanup(&timestamp)== TRUE) {
		return 0;
	} else
		return 1;
}
...
dessert_msg_t* aodv_create_rerr(_onlb_element_t** head, u_int16_t count) {
	...
	u_int8_t MAX_RERRDL_COUNT = (dessert_maxlen - sizeof(dessert_msg_t)
			- sizeof(struct ether_header)
			- sizeof(struct aodv_msg_rerr)
			- sizeof(struct aodv_msg_broadcast)) / DESSERT_MAXEXTDATALEN;

	while(*head != NULL && rerrdl_count < MAX_RERRDL_COUNT) {
		if (ext_el_num == 0) {
			dessert_msg_addext(msg, &ext, RERRDL_EXT_TYPE,
					((count >= max_dl_len)? max_dl_len : count) * ETH_ALEN);
			dl = ext->data;
		}
...
int aodv_periodic_scexecute(void *data, struct timeval *scheduled, struct timeval *interval) {
	...
	if (schedule_type == AODV_SC_REPEAT_RREQ) {
		// send out rreq
